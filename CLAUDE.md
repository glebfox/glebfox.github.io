...
- `images/photo.{avif,webp,jpg}` — 448² hero avatar served via `<picture>` (AVIF ~27K → WebP ~29K → JPEG ~75K fallback), generated with `npx sharp` at 448² from `images/photo.png` (1000² source, also used for the og card; no native image CLI installed); the picture occupies the right hero column on desktop and a separate block before the name on mobile
...
**Layout:** the content width is 63rem. The desktop hero has text on the left and a modest rectangular portrait on the right. Three equal work columns become aligned rows below 56rem and stacked blocks below 36rem. On mobile, the smaller portrait sits above the name and introduction, matching DOM order. Check mobile layouts with 200% root font size as well as the default size; headings and contact links must wrap without overlap or horizontal overflow. The introduction's vertical rule and amber segment repeat once beside "Currently exploring". The portrait has a warm padded frame, soft shadow, and a slight static tilt; a CSS filter gently reduces saturation without changing the source assets.
